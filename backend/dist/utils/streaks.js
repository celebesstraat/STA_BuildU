...
const calculateStreak = (progressUpdates) => {
    if (!progressUpdates || progressUpdates.length === 0) {
        return 0;
    }
    const sortedUpdates = progressUpdates.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    let streak = 0;
    let lastDate = new Date();
    const today = new Date();
    const todayString = today.toDateString();
    const hasActivityToday = sortedUpdates.some((update) => new Date(update.createdAt).toDateString() === todayString);
    if (hasActivityToday) {
        streak = 1;
        lastDate = today;
    }
