...
describe('Goals API', () => {
    beforeEach(() => {
        ...
        database_1.default.from.mockReturnValue(mockChainable);
    });
    describe('GET /', () => {
        test('should return all goals for the authenticated user', async () => {
            ...
            const mockFromReturn = database_1.default.from('goals');
            ...
            const res = await (0, supertest_1.default)(app).get('/goals');
      ...
      `);
        ...
        test('should return filtered goals for the authenticated user', async () => {
            ...
            mockFromReturn.error = null;
            mockFromReturn.count = mockGoals.length;
            const res = await (0, supertest_1.default)(app).get('/goals?status=active&category=career');
            expect(res.statusCode).toEqual(200);
            expect(res.body.success).toBe(true);
    ...
    describe('GET /:id', () => {
        test('should return a single goal by ID for the authenticated user', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get('/goals/goal1');
            expect(res.statusCode).toEqual(200);
            expect(res.body.success).toBe(true);
        ...
        test('should return 404 if goal not found', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get('/goals/nonexistent-goal');
    ...
    describe('POST /', () => {
        test('should create a new goal', async () => {
            ...
            const mockFromReturn = database_1.default.from('goals');
            mockFromReturn.insert.mockResolvedValue({ data: mockNewGoal, error: null });
            const res = await (0, supertest_1.default)(app).post('/goals').send(newGoalData);
        ...
        test('should create a new goal with milestones', async () => {
            ...
            const mockFromReturn = database_1.default.from('goals');
            mockFromReturn.insert.mockResolvedValueOnce({ data: mockNewGoal, error: null }); // For goal insert
            mockFromReturn.insert.mockResolvedValueOnce({ data: [], error: null }); // For milestones insert
            const res = await (0, supertest_1.default)(app).post('/goals').send(newGoalData);
            expect(res.statusCode).toEqual(201);
            expect(res.body.success).toBe(true);
        ...
        test('should return 400 if required fields are missing', async () => {
            ...
            const res = await (0, supertest_1.default)(app).post('/goals').send(invalidGoalData);
