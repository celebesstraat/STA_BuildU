...
router.post('/chat', async (req, res) => {
    try {
        ...
        let response = '';
        let suggestions = [];
        const { data: motivationalQuotes } = await database_1.default.from('motivational_content').select('content, author, category').eq('type', 'quote');
        const { data: goalSettingTips } = await database_1.default.from('motivational_content').select('title, content').eq('type', 'tip');
...
router.get('/tips', async (req, res) => {
    try {
        ...
        const { data: tips } = await database_1.default.from('motivational_content').select('title, content, category').eq('type', 'tip');
        if (!tips) {
            return res.status(500).json({
                success: false,
                error: 'Could not fetch motivational content'
